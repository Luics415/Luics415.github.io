# Avisos de terceros

Este documento acompaña al portafolio de Luics415 y resume componentes, servicios y derechos que no quedan reemplazados por la Licencia MIT del código original.

## Dependencias de software

La aplicación fue construida con tecnologías de código abierto, entre ellas:

- React;
- TanStack Router;
- Vite;
- TypeScript;
- Lucide Icons;
- utilidades y herramientas del ecosistema npm utilizadas durante el desarrollo y la compilación.

Cada paquete conserva su propia licencia, avisos de copyright y condiciones. La lista exacta de dependencias corresponde al proyecto fuente y a su archivo de bloqueo; los archivos de este repositorio son una salida compilada.

## Servicios de publicación

GitHub, GitHub Actions y GitHub Pages son productos de GitHub, Inc. Su uso está sujeto a sus términos, políticas y marcas correspondientes.

## Tipografías

El sitio puede solicitar tipografías desde Google Fonts. Las familias tipográficas conservan las licencias indicadas por sus respectivos autores y distribuidores.

## Proyectos y capturas

Los proyectos enlazados desde el portafolio mantienen licencias independientes. Sus capturas, nombres, datos, recursos y contenidos no se relicencian automáticamente bajo la Licencia MIT de este repositorio.

La ficha **Cubo Rubik** identifica su motor como una integración de la demostración pública [*The Cube* de bsehovac](https://codepen.io/bsehovac/pen/EMyWVv). Los patrones explicativos del portafolio son reconstrucciones conceptuales y no atribuyen ese motor a Luics415.

## Identidad de Luics415

La firma digital, el emblema de ancla con la letra “L”, los iconos, banners, grafitis y demás recursos de identidad de Luics415 no se ofrecen bajo la Licencia MIT salvo autorización expresa y escrita.

## Marcas y referencias culturales

*Arcane*, *League of Legends*, Riot Games, Fortiche, Netflix y cualquier otra marca citada pertenecen a sus respectivos titulares. La inspiración cromática o ambiental del portafolio no implica afiliación, patrocinio ni respaldo.

## Sin transferencia de derechos

La inclusión de un nombre, paquete, servicio, captura o enlace tiene fines informativos y de atribución. No transfiere derechos de propiedad intelectual ni modifica la licencia original del tercero correspondiente.
